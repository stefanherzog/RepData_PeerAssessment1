...
Calculate the mean steps taken for each interval

```{r}

per_interval <- summarize(group_by(dat, interval),
...
abline(h = interval_max_steps, col = "red")
```
...
plot(
    per_interval$interval, per_interval$avg_steps,
    type = "l",
    col = "blue",
    lwd = 2
)
...
per_interval_imp <- summarize(
    group_by(dat_imp, interval),
    avg_steps = mean(steps, na.rm = TRUE)
)

points(add = TRUE,
    per_interval_imp$interval, per_interval_imp$avg_steps,
    type = "l",
    col = "green",
    lwd = 2
)
